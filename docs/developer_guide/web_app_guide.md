...
* [Node.js and the node package manager (npm)](https://github.com/timetherewere/monero-javascript/blob/master/docs/developer_guide/prerequisite_installation_guides#node.js_and_npm)
  _If you followed the [getting started guide](https://moneroecosystem.org/monero-javascript/docs/developer-guide/getting_started.md), you should already have intalled node.js._
...
_Note_:
* Unlike create-react-app, the xmr-sample-app script will not automatically open a web browser. You must open the browser and navigate to "localhost:9100" manually.
* The script will not automatically refresh the server and page whenever a source file is modified.
...
The starter web application also serves as a demonstration of some basic monero-javascript methods (check the developer console to see the results) and a visual mock-up of a wallet application that a developer could potentially produce using the monero-javascript library.

Although this application _does_ show off a pretty slick Monero wallet design, it is not useful for building an offline wallet generator, so remove the extraneous code and files to make room for your wallet generator.

* Delete the sample wallet images:
rm -rf src/mockups
<!--
### Windows
rmdir /s /q src\mockups
-->
* Navigate to the "./src/" directory.
* Modify the index.html file so that it displays a blank page and runs the javascript code:
...
```
_Note that index.html references a javascript file called "xmr-sample-app.js" rather than index.html._ This is due to the fact that the build_browser_app shell script assembles the final web application image in the "browser_build" folder, where it saves the working copy of index.js under the file name "xmr-sample-app.js".

* Replace the contents of index.js with the node.js offline wallet generator from [part 1 of the getting started guide](https://github.com/monero-ecosystem/monero-javascript/blob/master/docs/developer_guide/getting_started.md):

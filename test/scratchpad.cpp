...
void open_wallet(string path, string password, int net_type) {
  cout << "open_wallet(" << path << ", " << password << ", " << net_type << ")" << endl;
  std::unique_ptr<tools::wallet2> wallet;
  wallet.reset(new tools::wallet2(cryptonote::network_type::STAGENET, 1, true));
  throw runtime_error("Not implemented");
}

void create_wallet_random(string language, int net_type) {
  tools::wallet2* wallet = new tools::wallet2(static_cast<cryptonote::network_type>(net_type), 1, true);
...
//  open_wallet(path, password, net_type);
//  create_wallet_random(language, net_type);

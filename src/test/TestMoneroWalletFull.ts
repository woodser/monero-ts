...
export default class TestMoneroWalletFull extends TestMoneroWalletCommon {
  ...
  async afterEach(currentTest) {
    ...
    let items = (await TestUtils.getDefaultFs()).readdirSync(TestUtils.TEST_WALLETS_DIR, "buffer");
    for (let item of items) {
      ...
      if (!found) (await TestUtils.getDefaultFs()).unlinkSync(TestUtils.TEST_WALLETS_DIR + "/" + item);
    }
  }
  ...
  protected testWalletFull() {
    ...
    describe("Tests specific to WebAssembly wallet", function() {
      ...
      it("Is compatible with monero-wallet-rpc wallet files", async function() {
        ...
        (await TestUtils.getDefaultFs()).unlinkSync(path);
...
//          assert(!(await wallet.isSynced()));  
      ...
      it("Can be closed", async function() {
        ...
        try {
          ...
          catch (e: any) { assert.equal(e.message, "Wallet is closed"); }
  ...
  protected static getRandomWalletPath() {
  ...
  protected static async testWalletEqualityOnChain(wallet1, wallet2) {

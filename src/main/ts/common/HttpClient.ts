...
export default class HttpClient {
  ...
  protected static async requestFetch(req) {
    ...
    let resp = await HttpClient.TASK_QUEUES[host].submit(async function() {
      return HttpClient.PROMISE_THROTTLES[host].add(function(opts) { return Request(opts); }.bind(this, opts));
    });
  ...
  protected static async requestXhr(req) {
   ...
   * MIT licensed.
   */
  protected static digestAuthRequest = function(method, url, username, password) {
    var self = this;

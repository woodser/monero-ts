...
self.getTransfers = async function(walletId, blockJsonQuery) {
  ...
  // deserialize query which is json string rooted at block
  let query = (new MoneroBlock(blockJsonQuery, MoneroBlock.DeserializationType.TX_QUERY).getTxs()[0] as MoneroTxQuery).getTransferQuery();
...
self.getOutputs = async function(walletId, blockJsonQuery) {
  ...
  // deserialize query which is json string rooted at block
  let query = (new MoneroBlock(blockJsonQuery, MoneroBlock.DeserializationType.TX_QUERY).getTxs()[0] as MoneroTxQuery).getOutputQuery();
